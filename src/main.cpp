...
#if defined(NDEBUG)
# error "TelleCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6d68675c260ea98e0a585c3f0907b0f60f50ef860c154373f00ff9c4e10b0033");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TelleCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TelleCoin Signed Message:\n";
...
{
    // TelleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 52500 blocks
    nSubsidy >>= (nHeight / 52500);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TelleCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TelleCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1524515430, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Jakarta 08 08 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ca015300971e9806562b759cc2a0ce1d5c723c4e3c4ac5a239f6e7875102d7125bf1a23981cfd53b865d022d557254898e234c5260fe7c917a2200a3964ab2e5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1524515430;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1676838;
        ...
        assert(block.hashMerkleRoot == uint256("0x2512a90ada1570aac31c70820e9e40ef3281114f7c1641bb27f3876300cb2c52"));
...
unsigned char pchMessageStart[4] = { 0xe8, 0x1b, 0x10, 0xfd }; // TelleCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TelleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TelleCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TelleCoinMiner(CWallet *pwallet)
{
    printf("TelleCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tellecoin-miner");
    ...
    try { loop {
        ...
        printf("Running TelleCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TelleCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TelleCoinMiner, pwallet));
}
